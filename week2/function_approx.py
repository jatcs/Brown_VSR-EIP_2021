...
function_target = lambda x: (5 + tf.sin(x) + tf.sin(2 * x) + tf.sin(3 * x) \
                            + tf.sin(4 * x)) \
...
train_X = ((domain[UPPER_BOUND] - domain[LOWER_BOUND]) \
            * np.random.random_sample(num_points)) + domain[LOWER_BOUND]
